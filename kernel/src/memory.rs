...
/// A FrameAllocator that returns usable frames
...
impl BootInfoFrameAllocator {
    ...
    ///
    /// SAFETY: The caller must gurantee that the passed
    /// memory map is valid. The main requirement is that all
    /// frames that are marked as `USABLE` in it are already
    /// unused.
